var d3clock = function (config) {
    ...
    */
    ...
    var setTZHours = function (hr) {
        if (hr !== undefined) hourOffset = hr;
    };
    var setTZMins = function (min) {
        if (min !== undefined) minOffset = min;
    };
    var setTZSeconds = function (sec) {
        if (sec !== undefined) secOffset = sec;
    };
    ...
    var faces = {
        sbb: {
            outerRing: { r: outerRadius * 1.05, stroke: "darkgray", strokeWidth: 2 },
            secondsRing: { r: outerRadius * 0, fill: "darkgray" },
            secondsInnermostRing: { r: outerRadius * 0, fill: "darkgray" },
        ...
        modern: {
            ...
            secondsRing: { r: outerRadius * 0.05, fill: "darkgray" },
            secondsInnermostRing: { r: (outerRadius * 0.05) / 2, fill: "darkgray" },
            tickUnit: (outerRadius * 0.0625) / 3,
            tickWidth: function (i) {
        ...
        braun: {
            ...
            secondsInnermostRing: { r: (outerRadius * 0.05) / 5, fill: "darkgray" },
            tickUnit: (outerRadius * 0.0625) / 3,
            tickWidth: function (i) {
        ...
        classic: {
            outerRing: { r: outerRadius * 1, fill: "darkgray", stroke: "#000", strokeWidth: 1 },
            secondsRing: { r: outerRadius * 0.04, fill: "darkgray" },
            secondsInnermostRing: { r: (outerRadius * 0.05) / 5, fill: "darkgray" },
            tickUnit: (outerRadius * 0.0625) / 3,
            tickWidth: function (i) {
    ...
    tickGroup = clockGroup.append("svg:g").selectAll(".tick").data(d3.range(60)).enter();
    ...
    tickGroup
        ...
        .attr("width", function (d, i) {
            return faceObj.tickWidth(i);
        })
        .attr("height", function (d, i) {
            return faceObj.tickHeight(i);
        })
        .attr("fill", function (d, i) {
            return faceObj.tickFill(i);
        })
    ...
    if (faceObj.tickText) {
        var radian = function (i) {
            return 6 * i * (Math.PI / 180) - (90 * Math.PI) / 180;
        };
        tickGroup
            .append("text")
            .attr("class", "tick")
    ...
    if (faceObj.tickTextRotated) {
        tickGroup
            ...
            .attr("class", "tick")
            .attr("x", function (i) {
                return faceObj.tickTextX(i);
            })
            .attr("y", function (i) {
                return -outerRadius + faceObj.tickHeight(i) * 2.5;
            })
            .attr("font-family", faceObj.tickTextRotated.fontFamily)
            .attr("font-size", faceObj.tickTextRotated.fontSize)
            .text(function (d, i) {
                return faceObj.tickTextRotated.fn(i);
            })
            .attr("transform", function (d, i) {
                return "rotate(" + i * 6 + ")," + faceObj.rotationTranslate(i);
            });
